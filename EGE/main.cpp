...
void test(Element* e){
	...
	if(e->ismousein()){
		...
		if(e->ishit()) {
			cout<<"HIT";
			if(statu == 1) e->set_variable(0,2);
		}
		else {
			cout<<"NOT_HIT";
			if(statu == 0) e->set_variable(0,1);
			else if(statu == 2) e->set_variable(0,3);
		}
	}
	else e->set_variable(0,0);
} 
void fun1(Element* e){
	e->move_down(1);
}
...
//		if(keymsg == (int)('a' + ('d' - 'a'))) e0.move_right(5);
	...
	cout<<e0.get_variable(0)<<" "<<e0.ismousein()<<endl; 
